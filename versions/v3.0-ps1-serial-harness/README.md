...
- D1 is the yellow-green firmware `STATUS` LED: KENTO `KT-0603YG` / LCSC `C2289`, driven by ESP32 module pin 26 on `STATUS_LED` through R13, 1 kΩ Yageo `RC0603FR-071KL` / `C22548`; polarity is unchanged.
- D6 is the red always-on 3V3 `PWR` LED: KENTO `KT-0603R` / `C2286`, through R29, 1 kΩ Yageo `RC0603FR-071KL` / `C22548`.
...
- J1/J2: Hong Cheng `HC-USB3.0-L168-ZP` / JLCPCB `C7501856`, used as keyed nine-contact **non-USB** harness interfaces. J2 carries the validated three-wire PS1 serial link.
- The official JLCPCB/EasyEDA footprint and 3D model are stored locally. All nine electrical-pad centers exactly match the prior routed pattern; only the connector shell slots, body/courtyard and visual model changed.
- J1 is at (8.025, 50.825) mm and J2 at (26.025, 50.825) mm in the vendor footprint coordinate frame. Their absolute signal-pad coordinates and all routed copper are unchanged, and both mating mouths face outward through the lower edge. J3 remains at (51.025, 9.625) mm, 90°, facing outward through the right edge.
...
J1 and J2 use the official Hong Cheng `HC-USB3.0-L168-ZP` / `C7501856`
through-hole pattern: nine 0.70 mm signal holes plus two 0.60 × 2.30 mm plated
shell slots. The 13.10 × 16.80 mm body fits at the existing lower edge without
a PCB cutout; its mouth intentionally overhangs the board while all signal pins
and both shell slots remain supported by the PCB. Board thickness remains 1.20 mm.

## 3D-model validation

The official `C7501856` WRL/STEP model uses zero offset, zero rotation and unit
scale in the vendor footprint frame. Direct front and perspective renders show
both bodies seated on the top surface, the two mouths facing outward, and the
through-hole tails/retention legs aligned to their pads. See the accepted
`renders/v3.0-*.png` inspection set.
