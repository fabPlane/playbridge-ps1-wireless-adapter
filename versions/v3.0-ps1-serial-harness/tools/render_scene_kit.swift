...
// Visualization-only reference/pin callouts requested for the V2.1 review
// images. These do not alter PCB fabrication data or connectivity.
func addBoardLabel(_ value:String, x:Double, z:Double, size:CGFloat = 2.5) {
    let text = SCNText(string:value, extrusionDepth:0.02)
    text.font = NSFont.systemFont(ofSize:size, weight:.semibold)
    text.flatness = 0.08
    let mat=SCNMaterial(); mat.diffuse.contents=NSColor.white; mat.emission.contents=NSColor(calibratedWhite:0.18,alpha:1); mat.lightingModel = .constant
    text.materials=[mat]
    let node=SCNNode(geometry:text)
    node.name="RENDER_LABEL_\(value)"
    let (mn,mx)=text.boundingBox
    node.pivot=SCNMatrix4MakeTranslation((mn.x+mx.x)/2,(mn.y+mx.y)/2,0)
    node.scale=SCNVector3(0.001,0.001,0.001)
    node.eulerAngles.x = -.pi/2
    node.position=SCNVector3(x/1000.0,0.00131,z/1000.0)
    scene.rootNode.addChildNode(node)
}
addBoardLabel("J1",x:4.5,z:47.0)
addBoardLabel("J2",x:22.5,z:47.0)
addBoardLabel("J3",x:49.0,z:16.5)
addBoardLabel("J4",x:50.0,z:31.7)
addBoardLabel("GND",x:49.0,z:22.0,size:1.5)
addBoardLabel("3V3",x:49.0,z:24.54,size:1.5)
addBoardLabel("TX",x:49.6,z:27.08,size:1.5)
addBoardLabel("RX",x:49.6,z:29.62,size:1.5)
...
if CommandLine.arguments.count >= 4,
   ...
   let items = root["items"] as? [[String:Any]] {
    let palette:[String:NSColor] = [
        ...
        ,"connector": NSColor(calibratedWhite:0.055,alpha:1)
    ]
    for item in items {
...
scene.rootNode.enumerateChildNodes { node, _ in
    if node.name?.hasPrefix("RENDER_ENVELOPE_") == true || node.name?.hasPrefix("RENDER_LABEL_") == true || node.name == "VISUALIZATION_SOLDER_MASK_GREEN" { return }
...
let views:[View] = [
    ...
    ,View(name:"j4-close-up", pos:SCNVector3(0.090,0.040,0.026), target:SCNVector3(0.053,0.003,0.026), ortho:false, scale:0)
]
